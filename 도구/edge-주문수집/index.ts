...
function 줄만들기(o: string[], 마스터: Record<string, any>, 판매처글: string) {
  ...
  전화칸손질(o);   // 🔴 원본 27칸이 로젠 송장으로 그대로 나간다
  ...
  const 줄: any = {
    ...
    단가: 숫자(o[자리["판매가"]]),
    수령인: 다듬기(o[자리["수령인"]]),
    수령인전화: 전화고르기(o[자리["핸드폰"]], o[자리["전화번호"]], o[자리["수령지전화"]],
      o[자리["주문자핸드폰"]], o[자리["주문자전화번호"]]),   // 🔴 받는사람 칸이 셋 다 비면 주문자 번호라도 쓴다
