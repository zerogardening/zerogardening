...
갈래상한 = {'상세페이지': 3 * 60 * 60, '카페24올리기': 80 * 60,
            '마켓품절': 30 * 60, '마켓판매': 30 * 60}
# 🔴 정해진 갈래는 Claude 의 긴 설명 대신 이 짧은 말로 답장·알림을 채운다 (우람님 9/23 「간결하게」)
갈래별완료말 = {'상세페이지': '이미지 갱신', '카페24올리기': '업로드',
             '상세페이지갱신': '상세페이지 갱신', '마켓품절': '품절 처리', '마켓판매': '판매 재개'}
# 🔴 카페24올리기 답장은 채널별로 한 줄씩 적으라고 시킨다(위 안내문 ⑤~⑧) — 그 줄에서
#    실패한 채널만 뽑아 「부분완료 (실패:롯데온)」식으로 붙인다 (우람님 9/23)
카페24채널들 = ['대표이미지', '롯데온', '진열', '쿠팡', '심폴']
실패낱말들 = ('실패', '못 했', '못했', '못 보냈', '못 올', '안 켰', '반려', '안 함', '안 감')


def 부분실패채널(답원문):
    실패 = []
    for 줄 in (답원문 or '').split('\n'):
        for 채널 in 카페24채널들:
            if 채널 in 줄 and any(k in 줄 for k in 실패낱말들) and 채널 not in 실패:
                실패.append(채널)
    return 실패
...
python3 "상품/_도구/심폴품절.py" 품절 %(코드)s --진짜

그 다음 이 한 줄.

python3 "상품/_도구/마켓플러스품절.py" 품절 %(코드)s --진짜

🔴 이 둘뿐이다. 다른 파일을 고치거나 다른 명령을 돌리지 마라.
🔴 출력에 🔴 가 있으면 **실패다.** 그 줄을 빠짐없이 옮겨 적어라. 다시 돌리지 마라 —
   마켓에 두 번 나가면 우람님이 손으로 되돌리셔야 한다.
🔴 자사몰(카페24)은 **이미 처리됐다.** 여기서 건드리지 마라.
🔴 심폴이 실패해도 마켓플러스는 돌려라 — 한쪽만 되는 게 둘 다 안 되는 것보다 낫다.
""",

    '마켓판매': """오픈마켓을 다시 판매중으로 되돌린다. **아래 한 줄만** 돌리고, 나온 글을 그대로 답장에 옮겨 적어라.

python3 "상품/_도구/심폴품절.py" 판매 %(코드)s --진짜
...
절차는 `상품/카페24-등록-메모.md` 에 다 적혀 있다 — **그 문서의 대응표대로** 한다.
  ...
  ① (⓪ 에서 못 찾았을 때만) `POST products` 로 새 상품을 등록한다(SEO.md 가 9칸의 유일한 원본이다)
     🔴 **원산지 세 칸을 POST 에 반드시 함께 넣는다** (2026-09-14 우람님 「빼먹지마」) —
        `origin_place_no: 40` (국내 > 경기도 > 용인시) · `origin_classification: "F"` · `made_in_code: "KR"`
        🔴🔴 **2026-09-20 — `40` 이 용인시다. 옛 값 `1798` 은 틀렸다.**
           오래도록 「40 = 빈 원산지」로 잘못 알고 173개를 전부 `1798` 로 덮어써 왔다.
           `1798` 은 관리자 원산지 드롭다운 어디에도 없는 값이라 **화면이 못 읽고 비어 보인다.**
           우람님이 「원산지가 비었다」고 잡으신 그 화면이 이것이다 — 값이 없어서가 아니었다.
        🔴 **API 가 값을 돌려줘도 화면은 빌 수 있다.** `GET` 이 `1798` 을 멀쩡히 돌려줘서
           도구가 「전부 채워졌다」고 한 달을 보고했다. 확인은 관리자 화면 3단(국내>경기도>용인시)으로 한다.
        🔴 등록 직후 `products/{번호}` 를 **한 건 조회**해 `origin_place_no` 가 40 인지 보고
           답장에 적어라(목록조회는 방금 넣은 값을 한 박자 늦게 준다).
           아니면 `python3 "상품/_도구/원산지.py" 채우기 {번호}` 로 고친 뒤 다시 본다.
...
def 한건():
    ...
    #    🔴 **헤드리스면 양보하지 않는다** (2026-09-18). 창이 안 뜨니 우람님 일을 가로챌 것이 없다 —
    #       양보하느라 5분씩 늦던 것이 사라진다.
    기다린초 = (time.time() * 1000 - ((것[1].get('보낸때')) or 0)) / 1000
    if not os.environ.get('ZG_헤드리스') and 유휴초() < 쉴유휴초 and 기다린초 < 못참는초:
    ...
    답원문, 자취 = 기록읽기(기록)
    답 = 답장뽑기(답원문, 자취)
    if 됐나 and 내용.get('갈래') in 갈래별완료말:
        이름 = 내용.get('폴더') or 내용.get('품목코드') or 줄id
        갈래 = 내용.get('갈래')
        실패채널 = 부분실패채널(답원문) if 갈래 == '카페24올리기' else []
        if 실패채널:
            답 = '%s %s 부분완료 (실패:%s)' % (이름, 갈래별완료말[갈래], '·'.join(실패채널))
        else:
            답 = '%s %s 완료' % (이름, 갈래별완료말[갈래])
    elif 됐나 is None:
