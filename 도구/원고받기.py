...
"""
import json, re, shutil, subprocess, sys, time, urllib.parse, urllib.request
...
# 🔴 2026-09-23 우람님 — 「v3에서 원고 고친 게 기준이 되어 카페24까지 이어지게」.
#    지금까진 여기서 만든 그림이 Supabase(앱 미리보기)까지만 갔다 — **카페24 실제 상세페이지는
#    안 바뀐다.** 우람님이 앱에서 고치고 확인하셔도 손님이 보는 페이지는 옛날 그대로였다는 뜻.
#    이미 등록된 상품이면(`창고주소.md`에 상품번호가 있으면) 상세설명까지 같이 올린다.
#    진열·판매·가격은 절대 안 건드린다 — `상세설명-올리기.py` 자체가 그 값을 안 본다.
상세올리기 = 뿌리 / '상품' / '_도구' / '상세설명-올리기.py'


def 카페24번호(방, 규=''):
    """🔴 2026-09-23 — 두 규격 품목은 `창고주소-{규격}.md` 를 먼저 본다.
       그전엔 뿌리의 `창고주소.md` 만 읽어서, 향등골나물-알바가 규격으로 갈리기 전의
       죽은 상품번호(208, 지금은 404)를 10·15 양쪽에 쓰려 들었다."""
    후보 = ([방 / ('창고주소-%s.md' % 규)] if 규 else []) + [방 / '창고주소.md']
    for 파일 in 후보:
        if not 파일.exists():
            continue
        m = re.search(r'상품번호\s*(\d+)', 파일.read_text(encoding='utf-8'))
        if m:
            return m.group(1)
    return ''
...
def 카페24반영(코드, 방):
    """이미 등록된 상품이면 방금 새로 뽑은 발행이미지를 카페24 상세설명에도 올린다.
    진열·판매·가격·이미지 URL 구조 외에는 건드리지 않는다(`상세설명-올리기.py` 보장)."""
    번호 = 카페24번호(방, 원고.규격(코드))
    if not 번호:
        return ''   # 아직 미등록 — 할 일 없다, 침묵한다(첫 등록은 사람·에이전트 몫)
    규 = 원고.규격(코드)
    제작 = 원고.제작본(방, 코드)
    이름 = 방.name + ('/' + 규 if (규 and 제작.parent.name == 규) else '')
    r = subprocess.run(['python3', str(상세올리기), 번호, 이름],
                        capture_output=True, text=True, timeout=120)
    if r.returncode != 0:
        끝 = [t for t in (r.stdout + r.stderr).strip().split('\n') if t.strip()]
        return '카페24 반영 실패 — ' + (끝[-1][:120] if 끝 else '까닭 모름')
    return '카페24 %s 상세설명 갱신됨' % 번호
