...
# 🔴 소급 방지 경계 — 06g-사진수집.js:12 의 `new Date(2026, 8, 9)` 와 같은 날이다. 옛 34품목은 대상이 아니다
경계 = int(time.mktime((2026, 9, 9, 0, 0, 0, 0, 0, -1)) * 1000)
...
def 주고받기():
    ...
    for 코드, 방 in sorted(코드표.items()):
        ...
        if (머리.get('만들기요청') or 0) > (머리.get('맥시각') or 0):
            ...
            머리고침 += 1
            print('  📄 %-24s 제작.html %s' % (방.name, '있음' if 원고.제작본(방).exists() else '없음'))
