...
def 크기뗀이름(이름):
    """구근 둘레 등급(`12+` · `(6+)` · `24+`)을 뗀다 — 상품명에 두면 손님이 수량과 헷갈리신다
       (2026-09-15 우람님 「12+같은 구근 크기는 상품명에서 빼줘」).
       `+` 가 반드시 붙어 있어야 지운다 — 화분묘 이름의 숫자는 건드리지 않는다."""
    return re.sub(r'\s*\(?\s*\d+\s*\+\s*\)?', '', str(이름 or '')).strip()


def 상품명(내):
    """카페24·심폴에 올라갈 이름. SEO.md 가 유일한 원본이지만 그 출발점이 이 줄이다.
       🔴 구근은 화분묘가 아니다 — 예전엔 규격 0 이 그대로 새어 `… 0cm화분` 이 나갔고,
          SEO 쓰는 쪽이 매번 손으로 고쳐 쓰고 있었다(알리움 SEO.md 에 그 메모가 남아 있다).
       판매단위가 2 이상이면 뒤에 `(5개)` 를 붙인다 — 한 번에 그만큼 보내드린다는 뜻이다."""
    묶음 = int(내.get('판매단위') or 1)
    if 구근인가(내):
        이름 = 크기뗀이름(내.get('유통명'))
        if '구근' not in 이름:
            이름 += ' 구근'
    else:
        이름 = '{} {}cm화분'.format(str(내.get('유통명') or '').strip(), 내.get('규격cm', ''))
    return 이름 + (' ({}개)'.format(묶음) if 묶음 > 1 else '')
...
def 폼줄(내용):
    ...
    return ('\n> 🔴 **구근이다 — 상세페이지 폼이 화분묘와 다르다.**\n'
            ...
            '> 🔴 **상품명은 위 표의 「상품명」 칸을 그대로 쓴다.** 「화분 규격」 칸(`0cm화분`)은 내부 파싱용이라\n'
            '>    상품명에 쓰면 안 된다. 구근 둘레(`12+`)도 이미 빼 두었다 — 도로 붙이지 않는다.\n')
