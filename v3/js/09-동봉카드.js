...
(function (ZG) {
  ...
  function 판들만들기(건들) {
    ...
    건들.forEach(function (g, i) {
      ...
      /* 로젠 파일을 뽑을 때 매긴 출력번호가 있으면 그 번호를 쓴다 — 송장 E열의 (12)와 같은 숫자가 된다.
         아직 안 뽑았으면 지금까지처럼 들어온 순서다 */
      var 번호 = Number((g.줄들 || [])[0] && g.줄들[0].출력번호) || (i + 1);
      /* 🔴 한 손님에게 상자가 둘 이상이면(08i 「송장 한 장 더」) 카드도 상자 수만큼 뽑아
         1-1·1-2 로 나눠 찍는다 — 로젠 송장 E열의 번호(08i.번호글)와 반드시 같아야 한다(8/10 지시).
         상자수 필드가 없거나 1이면(옛 주문·손질 없음) 지금까지처럼 한 벌만 찍는다 — 회귀 없음.
         품목을 상자별로 나누는 기능은 아직 없어(설계 §5-4) 같은 내용을 상자 수만큼 그대로 복제한다 */
      var 상자수 = Math.max(1, Number((g.줄들 || [])[0] && g.줄들[0].상자수) || 1);
      for (var 상자 = 1; 상자 <= 상자수; 상자++) {
        var 카드번호 = 상자수 >= 2 ? (번호 + '-' + 상자) : 번호;
        for (var 쪽 = 1; 쪽 <= 총쪽; 쪽++) {
          판들.push(판만들기(것들.slice((쪽 - 1) * 한장, 쪽 * 한장), 카드번호, 쪽, 총쪽, s));
        }
      }
    });
    return 판들;
