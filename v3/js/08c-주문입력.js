...
(function (ZG) {
  ...
  function 수동그리기(칸, 상태) {
    ...
    // 고객관리에서 이름·주소를 채워 열 수 있다. 🔴 한 번 쓰고 버린다 —
    // 안 버리면 다음에 수동주문을 열 때 남의 이름이 채워져 있다 (11단계 설계 §7)
    var 사람 = 사람칸(상태.채울값 || null);
    상태.채울값 = null;
    칸.appendChild(사람.요소);
  ...
  function 사람칸(g) {
    ...
    var 주소 = 입력칸(g ? (g.주소 || '') : '');
    /* 우편번호는 선택이다 — 막지않나() 에 넣지 않는다.
       적어 두면 고객이 이름+우편번호로 묶여 온라인 주문과 한 줄이 된다 (11단계 설계 §2) */
    var 우편 = 입력칸(g ? (g.우편번호 || '') : '', 'text', 'inp num');
    var 우편밭 = 밭('우편번호', 우편);
    우편밭.style.flex = '0 0 118px';
    ...
    return {
      요소: 요소, 이름: 이름, 전화: 전화, 주소: 주소, 우편: 우편,
      값: function () {
        return {
          이름: 이름.value.trim(), 전화: 전화.value.trim(),
          주소: 주소.value.trim(), 우편: 우편.value.trim()
        };
      }
    };
  }
  ...
  function 고쳐넣기(g, 사람, 묶음) {
    ...
    카드들.forEach(function (st) {
      ...
      if (st.id) {
        ...
        // 🔴 빈 우편번호로 덮으면 로젠 송장(08i)·재구매표가 쓰는 값이 날아간다
        if (것.우편) 바뀔것.우편번호 = 것.우편;
        바뀔것.묶음키 = ZG.주문자료.묶음키(바뀔것);   // 묶음키는 이름·전화·주소뿐이다 (08a:40)
        수정표시(옛줄[st.id], 바뀔것);
