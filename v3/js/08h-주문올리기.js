...
(function (ZG) {
  ...
  function 읽기결과(행들) {
    ...
    /* 🔴 판매처를 안 가리고 짝표를 태운다 (2026-08-10 우람님 지시) — 심폴은 여태처럼 짝이 전부고,
       카페24는 저절로 안 붙은 줄만 짝으로 메운다(붙은 줄은 안 덮는다, 08j.되붙이기 주석) */
    ZG.짝.되붙이기(항목들);
  ...
  function 미리보기창() {
    ...
    /* 짝창은 판매처를 안 가린다. 지을 짝도 지어 둔 짝도 없으면 null 이라 안 그려진다 (08k) */
    var 짝상자 = ZG.짝창.상자(상태.항목들, { 바뀌면: 짝바뀜 });
    if (짝상자) 몸.push(짝상자);
    /* 🔴 원본코드가 빈칸인 줄은 기억할 열쇠가 없어 짝을 못 짓는다 (우람님 확정 8/10) —
       카페24에 자체품목코드를 채우셔야 풀린다. 그 줄만 옛 안내로 남긴다 */
    var 열쇠없음 = 코드없음.filter(function (것) { return !것.줄.원본코드; });
    if (열쇠없음.length) 몸.push(코드없음상자(열쇠없음));
    if (이미) 몸.push(이미상자());
    if (날짜틀림) 몸.push(날짜상자());
  ...
  function 짝바뀜() { ZG.짝.되붙이기(상태.항목들); 그리기(); }
    ...
    var 몸 = [
      ...
      상자('회색', '⬇ 로젠 대량등록 파일', '올리신 파일 그대로 만듭니다', [
        표틀(['170px', ''], [{ t: '파일에 들어갈 것' }, { t: '내용', r: true }], [
          ...
          만들기('tr', {}, [만들기('td', { text: '출력번호' }), 만들기('td', { class: 'r', text: '1 ~ ' + 준비.원묶음수 + ' · 동봉카드 번호와 같습니다' })])
        ]),
        만들기('div', { class: 'allrow' }, [
