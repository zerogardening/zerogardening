...
(function (ZG) {
  ...
  function 원본27(행, 자리) {
    ...
    /* 🔴 08i-로젠파일.js 는 이 27칸을 그대로 송장에 찍고, 로젠은 '이름＋연락처＋주소'가
       같은 줄만 한 송장으로 묶는다. 심폴은 수령지전화가 늘 비고 핸드폰이 '02--' 라
       연락처가 성립하지 않아 품목마다 송장이 따로 뽑혔다(2026-09-21 우람님 신고).
       쓸 수 있는 번호를 골라 세워 준다 — 줄마다 같은 값이 된다 */
    var 좋은 = 연락처(행, 자리);
  ...
  function 읽기결과(행들) {
    ...
    var 당 = !심 && ZG.당근 && ZG.당근.감지(행들[0]);
    var 머리;
    if (심 || 당) {
      var 바꾼 = 심 ? ZG.심폴.바꾸기(행들, 심) : ZG.당근.바꾸기(행들, 당);
      행들 = 바꾼.행들;
      머리 = { 자리: 바꾼.자리, 없는칸: [], 모르는: [], 우편없음: false, 심폴: !!심, 당근: !!당 };
    } else {
      머리 = 머리읽기(행들[0]);
      if (머리.없는칸.length) {
        return { ok: false, 오류: '카페24 · 심폴 · 당근 주문 파일이 아닌 것 같습니다 — 못 찾은 칸: ' + 머리.없는칸.join(' · ') };
      }
    }
  ...
  /* 🔴 원본 27칸이 없는 줄(수동주문 · 심폴 · 옛 주문)을 저장된 값으로 세운다 (우람님 지시 8/19).
     지어내는 게 아니다 — 주문줄에 이미 있는 값만 제자리에 옮긴다.
     받는 사람을 못 적으면 택배가 못 간다. 수령인·주소 둘 중 하나라도 비면 세우지 않고 뺀다.
     08i 가 덮어쓰는 칸(E상품명 · J운임 · K택배수량)은 여기서 채우지 않는다. */
  function 세운27(r) {
    var 이름 = 다듬기(r.수령인), 주소 = 다듬기(r.수령인주소);
    if (!이름 || !주소) return null;
    var o = 칸이름.map(function () { return ''; });
    o[0] = 다듬기(r.판매처);                             // 쇼핑몰 — 파일에서 판매처를 알아보시라고
    o[2] = 다듬기(r.주문번호);
    o[3] = 다듬기(r.주문일);
    o[4] = 다듬기(r.유통명);                             // 품목코드가 있으면 08i 가 유통명+cm+수량으로 다시 쓴다
    o[6] = 다듬기(r.옵션원문);
    o[7] = 다듬기(r.품목코드 || r.원본코드);
    o[11] = String(Number(r.단가) || 0);                 // L 판매가
    o[12] = String(Number(r.주문수량) || 0);             // M 수량 — 08i 는 옵션입수까지 곱한 자료().수량() 을 따로 쓴다
    o[13] = 이름;
    o[14] = 다듬기(r.우편번호);
    o[15] = 주소;
    o[16] = o[18] = 다듬기(r.수령인전화);                // 수령지전화·핸드폰 — 로젠 합포장 기준이 연락처를 본다
                                                         // 🔴 08i 그룹키도 이 자리를 본다
    o[19] = 다듬기(r.배송메모);
    o[25] = '0';                                         // 옵션추가가격
    return o;
  ...
  /* ══ 엑셀 없이 — 이미 저장된 주문줄로 바로 로젠 파일 (우람님 지시 8/10) ══
     ...
     🔴 원본이 없는 줄은 세운27() 로 세워 같이 넣는다 (8/19). 수령인·주소가 없는 줄만 뺀다. */
  function 로젠만들기(줄들) {
    if (창) return;
    var 전부 = 줄들 || [];
    var 항목들 = [], 빠짐 = 0, 세움 = 0;
    전부.forEach(function (r) {
      if (!r) return;
      if (r.원본 && r.원본.length) {
        항목들.push({ 원본: r.원본, 줄: r, id: r.id || '', 갈래: '이미' });
        return;
      }
      var o = 세운27(r);
      if (!o) { 빠짐++; return; }
      세움++;
      항목들.push({ 원본: o, 줄: r, id: r.id || '', 갈래: '이미' });
    });
    if (!항목들.length) {
      u.토스트('로젠 파일로 만들 수 있는 주문이 없습니다 — 수령인과 주소가 있어야 합니다');
      return;
    }
    if (세움) u.토스트(세움 + '줄은 저장된 주소로 세워 넣었습니다 (수동주문 · 심폴 · 옛 주문)');
    if (빠짐) u.토스트(빠짐 + '줄은 수령인이나 주소가 없어 뺐습니다');
    /* 🔴 품목코드가 안 붙은 줄은 08i.상품글 이 심폴·카페24 상품명을 그대로 찍는다
       (「제로가드닝 밤프톤 1개」처럼 판매처 제목이 송장에 나간다).
       규격을 지어낼 수는 없으니 짝을 먼저 붙이시라고 알린다 (2026-09-21 우람님 신고) */
    var 짝없음 = 항목들.filter(function (것) { return !것.줄.품목코드; }).length;
    if (짝없음) u.토스트('⚠ ' + 짝없음 + '줄은 품목코드가 안 붙었습니다 — 송장에 판매처 상품명이 그대로 찍힙니다. 짝을 먼저 붙여 주세요');
  ...
  function 고르기창() {
    ...
    if (상태.파일) {
      ...
      고른칸 = 만들기('div', { class: 'drop' }, [
        만들기('span', { class: 'ic', text: '📄' }),
        만들기('div', { class: 't', text: '카페24 · 심폴 · 당근 주문 파일' }),
    ...
    껍데기({
      제목: '주문 올리기', 힌트: '카페24 · 심폴 · 당근 주문 파일', 좁게: true,
      몸: [
        고른칸,
  ...
  function 미리보기창() {
    ...
    껍데기({
      ...
      힌트: (상태.심폴 ? '심폴 · ' : 상태.당근 ? '당근 · ' : '') + 상태.파일명 + ' · ' + 상태.항목들.length + '줄 읽음' + (상태.무시 ? ' (무시 ' + 상태.무시 + '줄)' : ''),
      몸: 몸,
      발: [
  ...
  /* ══ 이미 송장을 뽑은 줄은 로젠 파일에서 뺀다 (우람님 지시 8/19) ══
     오전에 뽑고 오후에 추가 주문을 올리면 카페24 내보내기에 오전 건이 다시 섞여 나온다.
     그대로 로젠에 올리면 같은 주문에 송장이 두 번 나간다.
     🔴 「뽑았다」의 증거는 주문줄의 출력번호다 — 08i.번호되쓰기 가 파일을 실제로
        내려받았을 때만 박는다. 저장만 하고 안 뽑은 줄은 그대로 들어간다.
     🔴 재발행 경로(로젠만들기)는 일부러 다시 뽑는 것이라 거르지 않는다. */
  function 로젠항목() {
    if (상태.재발행) return 상태.항목들;
    var 나감 = {};
    ZG.저장소.읽기(ZG.저장소.키.주문).forEach(function (r) {
      if (r.id && Number(r.출력번호)) 나감[r.id] = 1;
    });
    return 상태.항목들.filter(function (것) { return !(것.id && 나감[것.id]); });
  }
  ...
  function 완료창() {
    ...
    var 묶음id = 회차id();
    /* 🔴 한 번만 거른다 — 로젠항목()이 주문 1MB 를 다시 파싱한다(운임손질이 준비하기를 여러 번 부른다) */
    var 로젠것들 = 로젠항목();
    var 뽑은거름 = 상태.항목들.length - 로젠것들.length;
    function 준비하기() { return ZG.로젠파일.준비(로젠것들, { 묶음id: 묶음id }); }
    var 준비 = 준비하기();
    var 번호들 = 준비.그룹들.filter(function (g) { return !g.추가; }).map(function (g) { return g.출력번호; });
    var 번호글 = 번호들.length
      ? (Math.min.apply(null, 번호들) + ' ~ ' + Math.max.apply(null, 번호들) + ' · 동봉카드 번호와 같습니다')
      : '없음';
    var 이름 = 파일이름();
      ...
      상자('회색', '⬇ 로젠 대량등록 파일', '올리신 파일 그대로 만듭니다', [
        표틀(['170px', ''], [{ t: '파일에 들어갈 것' }, { t: '내용', r: true }], [
          ...
          만들기('tr', {}, [만들기('td', { text: '박스운임 (J열)' }),
            만들기('td', { class: 'r', text: '송장 한 장 값이 들어갑니다 (박스를 나누면 송장 줄이 늘어납니다)' })]),
          ...
          만들기('tr', {}, [만들기('td', { text: '출력번호' }), 만들기('td', { class: 'r', text: 번호글 })]),
          만들기('tr', {}, [만들기('td', { text: '송장 이미 뽑은 줄' }),
            만들기('td', { class: 'r', html: 뽑은거름
              ? '<span style="' + 톤표.노랑글 + '">' + 뽑은거름 + '줄 뺐습니다</span> — 송장이 두 번 나가지 않습니다'
              : '없습니다' })])
        ]),
        만들기('div', { class: 'allrow' }, [
    ...
    껍데기({
      ...
      발: [
        ...
        단추('⬇ 로젠 대량등록 파일 내려받기', function () { 내려받기(이름, 묶음id, 로젠것들, 뽑은거름); }, 'main')
      ]
    });
  }

  function 내려받기(이름, 묶음id, 것들, 뽑은거름) {
    var 수 = ZG.로젠파일.내보내기(것들 || 상태.항목들, 이름, { 묶음id: 묶음id });
    if (!수) {
      /* 전부 이미 뽑은 줄이면 막다른 길로 두지 않는다 — 다시 뽑는 길을 알려준다 */
      u.토스트(뽑은거름
        ? '전부 이미 송장을 뽑은 주문입니다 — 다시 뽑으시려면 주문 목록에서 회차나 줄을 고르고 ⬆주문올리기를 누르세요'
        : '로젠 파일에 넣을 줄이 없습니다.');
      return;
    }
  ...
  ZG.주문올리기 = {
    열기: 열기, 닫기: 닫기, 로젠만들기: 로젠만들기, 세운27: 세운27,
