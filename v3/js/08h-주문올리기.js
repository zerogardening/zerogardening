...
(function (ZG) {
  ...
  function 읽기결과(행들) {
    ...
    var 머리 = 머리읽기(행들[0]);
    if (머리.없는칸.length) {
      return { ok: false, 오류: '카페24 주문 파일이 아닌 것 같습니다 — 못 찾은 칸: ' + 머리.없는칸.join(' · ') };
    }
  ...
  function 고르기창() {
    ...
    if (상태.파일) {
      ...
      고른칸 = 만들기('div', { class: 'drop' }, [
        만들기('span', { class: 'ic', text: '📄' }),
        만들기('div', { class: 't', text: '카페24에서 받은 주문 파일을 고르세요' }),
    ...
    껍데기({
      제목: '주문 올리기', 힌트: '카페24 주문 CSV', 좁게: true,
      몸: [
        고른칸,
  ...
  function 미리보기창() {
    ...
    껍데기({
      ...
      힌트: 상태.파일명 + ' · ' + 상태.항목들.length + '줄 읽음' + (상태.무시 ? ' (무시 ' + 상태.무시 + '줄)' : ''),
      몸: 몸,
      발: [
  ...
  /* 재발행이라 저장한 묶음이 없으면 이미 있던 줄의 묶음id 를, 그것도 없으면 오늘 1회차로 이름 붙인다 */
  function 파일이름() {
  ...
  function 완료창() {
      ...
      상자('회색', '⬇ 로젠 대량등록 파일', '올리신 파일 그대로 만듭니다', [
        표틀(['170px', ''], [{ t: '파일에 들어갈 것' }, { t: '내용', r: true }], [
          ...
          만들기('tr', {}, [만들기('td', { text: '박스운임 (J열)' }),
            만들기('td', { class: 'r', text: 준비.확인요망 ? 준비.확인요망 + '묶음이 「확인요망」 — 1박스를 넘을 수 있습니다' : '전부 자동 계산됐습니다' })]),
    ...
    껍데기({
      ...
      몸: 몸,
      발: [
        만들기('span', { class: 'fname', text: '내려받을 파일 — ' + 이름 + ' · ' + 준비.줄수 + '줄' }),
        만들기('span', { class: 'spacer' }), 단추('닫기', 닫기),
        단추('⬇ 로젠 대량등록 파일 내려받기', function () { 내려받기(이름); }, 'main')
      ]
    });
  }

  function 내려받기(이름) {
    var 수 = ZG.로젠파일.내보내기(상태.항목들, 이름);
