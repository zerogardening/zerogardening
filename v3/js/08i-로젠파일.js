...
(function (ZG) {
  ...
  function 추가줄(g) {
    ...
    o[C.상품명] = 앞머리(g.출력번호, 0) + g.차수 + '번 송장 · ' + 요약글(원);
  ...
  function 추가붙이기(그룹들, 손질표) {
    ...
    그룹들.forEach(function (g) {
      결과.push(g);
      (더[g.키] || []).forEach(function (손) {
        var p = {
          키: g.키, 차수: Number(손.차수) || 2, 추가: true, 원: g,
  ...
  function 준비(항목들, 옵션) {
    var r = 만들기(항목들, 옵션);
    var 확인 = 0, 코드없음 = 0, 박스합 = 0, 손댐 = 0, 운임합계 = 0;
    ...
    return {
      줄수: r.표.length, 묶음수: r.그룹들.length, 박스합: 박스합,
