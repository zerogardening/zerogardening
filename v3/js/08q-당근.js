/* 08q-당근 — 당근마켓 주문내역 파일 자료층. 화면 없다. 08j-심폴 과 같은 수를 쓴다.
   ...
   🔴 당근 파일에는 **수량 칸이 아예 없다.** 14칸 어디에도 없다 —
      **수량은 상품명 꽁무니에 「3개」로 붙어 온다**(2026-09-03 우람님). 거기서 떼어 쓴다.
   🔴 상품코드도 없다. 그래서 짝(ZG.짝)의 열쇠를 **수량을 뗀 상품명**으로 쓴다.
      🔴 수량을 안 떼면 「…모종 1개」와 「…모종 3개」가 딴 상품이 돼 짝을 몇 번이고 다시 지으셔야 한다.
      제목 자체를 고치시면 짝이 풀리고 다시 지으셔야 하는 건 그대로다.
...
(function (ZG) {
  ...
  var 지도 = [
    '@당근', null, '주문번호', '@일자', '@상품명키', '@상품명키', '@빈', '@상품명키',
    null, null, '주문상태', '가격', '@수량', '주문자명', '우편번호', '배송주소',
  ...
  /* 「제로가드닝 팜파스 푸밀라 (그라스) 12cm화분 모종 3개」 → { 이름:'…모종', 수량:3 }
     🔴 꽁무니의 「N개」만 뗀다. 「12cm화분」처럼 가운데 숫자는 건드리지 않는다.
     못 찾으면 이름은 그대로, 수량 1 — 사라지는 주문은 없다 */
  function 이름과수량(원문) {
    var s = 다듬기(원문);
    var m = s.match(/^(.*?)[\s·]*(\d+)\s*개$/);
    if (!m || !m[1]) return { 이름: s, 수량: 1 };
    return { 이름: m[1].trim(), 수량: Number(m[2]) || 1 };
  }
  ...
  ZG.당근 = { 감지: 감지, 바꾸기: 바꾸기, 날짜글: 날짜글, 이름과수량: 이름과수량 };
})(window.ZG);
