...
(function (ZG) {
  ...
  function 짐싸기(id) {
    var r = 기록찾기(id);
    if (!r) return null;
    /* 🔴 판매단위·쿠팡단위는 입고 기록이 아니라 품목 것이다 — 품목에서 읽고 저장도 품목에 한다
       (2026-09-15 우람님 「판매단위 입력칸 안생김」 — 등록 화면에만 넣었었다) */
    var 품 = ZG.저장소.읽기(ZG.저장소.키.품목).find(function (p) { return p.품목코드 === r.품목코드; }) || {};
    return {
      기록: r, 오류: {},
