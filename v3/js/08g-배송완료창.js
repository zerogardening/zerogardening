...
(function (ZG) {
  ...
  function 카페24새로() {
    ...
    var 속 = [만들기('span', {
      html: 짝들.length
        ? '이 파일에서 카페24 주문 <b>' + 짝들.length + '건</b>을 카페24에도 발송처리할 수 있습니다.' +
          (빠짐.length ? '<br>' + 빠짐.join(' · ') + '은 대상이 아닙니다.' : '')
        : '이 파일에 카페24 발송처리 대상이 없습니다.' + (빠짐.length ? '<br>' + 빠짐.join(' · ') + '은 대상이 아닙니다.' : '')
    })];

    if (짝들.length) {
      var 버튼 = 단추('☁ 카페24 발송처리 하기', function () {
    ...
    자리.appendChild(만들기('div', { class: 'needbox c24' }, [
      만들기('h4', {}, [
        ...
        만들기('span', { class: 'why', text: '심폴은 직접 발송처리하셔야 합니다' })
      ]),
