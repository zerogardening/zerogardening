...
(function (ZG) {
  ...
  /* 구역 머리에 다는 이름. 2·3 구역은 품목마다 소제목이 달라 값이 있으면 그것을 쓴다.
     🔴 구근은 이름이 통째로 다르다(3구역이 잎이 아니라 심는 법) — ZG.상세폼 이 가른다 (2026-09-14).
        품목을 안 주면 화분묘 것을 준다. 옛 호출부를 그대로 살려 두려는 것이다. */
  function 구역이름(품목) {
  ...
  /* 🔴 창고 폴더는 `발행` 이 아니라 `pub` 이다 — Storage 키에 한글을 못 쓴다(`InvalidKey`).
     설계는 `발행/` 이었는데 신규 입고가 0건이라 한 번도 안 올려 봐서 2026-09-14 까지 몰랐다.
     맥 쪽 `원고받기.py` 의 창고올리기 와 **반드시 같아야 한다.** */
  function 발행훑기(코드) { return ZG.사진수집.사진훑기(코드 + '/pub'); }
  function 발행주소(코드, 번, 때) { return ZG.사진수집.주소(코드, 'pub/' + 번 + '.jpg', 때); }
