...
(function (ZG) {
  ...
  /* 상한 근거 — product-writer.md 「글자수 상한」 = 상품/_필드정의.md:205.
     🔴 5구역은 화분묘 130 · **구근 230** 이다(아래 `폼상한`). 83 으로 잡으면 실측 32품목 중 11품목이
        열자마자 저장이 막힌다(설계 §1). */
  /* 🔴 2026-09-23 우람님 「글 나오는 자리는 다 고칠 수 있어야 한다」 —
     구근은 3구역이 3항목(언제 심나요·깊이와 방향·어디에 심나요)이고 6구역에 발송설명 글이 있다.
     화분묘 6구역은 진짜로 사진뿐이다. **폼마다 칸이 다르다** — 도구/원고파일.py 의 `칸목록()` 과 같아야 한다. */
  var 칸들 = [
    { 칸: '1',   구역: '1', 이름: '①', 소제목: false, 상한: 160 },
    { 칸: '2',   구역: '2', 이름: '②', 소제목: true,  상한: 130 },
    { 칸: '3',   구역: '3', 이름: '③', 소제목: true,  상한: 130 },
    { 칸: '3-1', 구역: '3', 이름: '①', 소제목: true,  상한: 190 },
    { 칸: '3-2', 구역: '3', 이름: '②', 소제목: true,  상한: 190 },
    { 칸: '3-3', 구역: '3', 이름: '③', 소제목: true,  상한: 190 },
    { 칸: '4-1', 구역: '4', 이름: '①', 소제목: true,  상한: 190 },
    { 칸: '4-2', 구역: '4', 이름: '②', 소제목: true,  상한: 190 },
    { 칸: '4-3', 구역: '4', 이름: '③', 소제목: true,  상한: 190 },
    /* 🔴 ④는 있는 품목에만 있다(아스타 베스차토·아폴로·피터해리슨). 없는 품목에선 아예 안 그린다 */
    { 칸: '4-4', 구역: '4', 이름: '④', 소제목: true,  상한: 190 },
    { 칸: '5',   구역: '5', 이름: '⑤', 소제목: false, 상한: 130 },
    { 칸: '6',   구역: '6', 이름: '⑥', 소제목: false, 상한: 130 },
    /* 🔴 2026-09-23 — 상세페이지에 가장 크게 찍히는데 여태 칸이 없던 한 줄짜리들 (15단계B §3).
       「12cm+」·「둘레 12cm 이상」·「더블 얼리」·「10-15cm」·「40-50cm」.
       우람님 「구근 크기가 편집이 안 된다」의 절반이 여기였다. 도구/원고파일.py 와 이름이 같아야 한다 */
    { 칸: '2-계열', 구역: '2', 이름: '계열',      소제목: false, 상한: 20 },
    { 칸: '2-둘레', 구역: '2', 이름: '구근 둘레',  소제목: false, 상한: 20 },
    { 칸: '3-깊이', 구역: '3', 이름: '심는 깊이',  소제목: false, 상한: 20 },
    { 칸: '3-간격', 구역: '3', 이름: '포기 간격',  소제목: false, 상한: 20 },
    { 칸: '5-규격', 구역: '5', 이름: '구근 크기',  소제목: true,  상한: 20 },
    { 칸: '5-간격', 구역: '5', 이름: '식재 간격',  소제목: false, 상한: 20 }
  ];

  /* 🔴 상한은 폼마다 다를 수 있다. 구근 5구역은 도해 밑 해설문이라 화분묘 「식재 간격」 한두 줄과 성격이 다르다 —
     실측 최대 227 로 15품목이 **전부** 130 을 넘어 한 글자만 고쳐도 저장이 통째로 거부됐다 (15단계B §1).
     `상품/_필드정의.md` 의 상한 표와 같은 값이어야 한다. */
  var 폼상한 = { 구근: { '5': 230 } };

  var 폼칸들 = {
    화분묘: ['1', '2', '3', '4-1', '4-2', '4-3', '4-4', '5', '5-간격'],
    구근:  ['1', '2', '2-계열', '2-둘레', '3-1', '3-2', '3-3', '3-깊이', '3-간격',
            '4-1', '4-2', '4-3', '5', '5-규격', '6']
  };

  /* 그 품목이 **가진** 칸만, 화면에 놓을 차례대로. 품목을 안 주면 화분묘 것을 준다(옛 호출부) */
  function 칸들폼(품목) {
    var 폼 = (ZG.상세폼 && ZG.상세폼.폼(품목)) || '화분묘';
    return 폼칸들[폼].map(칸찾기);
  }
  ...
  function 상한(칸, 품목) {
    var 폼 = (ZG.상세폼 && ZG.상세폼.폼(품목)) || '화분묘';
    var 덮 = 폼상한[폼];
    if (덮 && 덮[칸] != null) return 덮[칸];
    var 정 = 칸찾기(칸);
    return 정 ? 정.상한 : 0;
  }

  function 넘침(칸, 지금본문, 원래본문, 품목) {
    if (!칸찾기(칸)) return 0;
    var 웃 = 상한(칸, 품목);
    var 셈 = 글자수(지금본문);
    if (셈 <= 웃) return 0;
  ...
  /* 머리줄 하나 + 글칸 전부. 🔴 폼을 안 가리고 다 묻는다 — 없는 칸은 줄이 없어 안 온다 */
  function id들(코드) {
  ...
  ZG.원고자료 = {
    칸들: 칸들, 칸들폼: 칸들폼, 구역이름: 구역이름, 칸찾기: 칸찾기, 글자수: 글자수,
    상한: 상한, 넘침: 넘침,
