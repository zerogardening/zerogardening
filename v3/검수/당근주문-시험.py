...
# 🔴 당근 파일에는 수량 칸이 없다 → 상품명 꽁무니의 「N개」가 수량이다(우람님 9/3).
#    상품코드도 없어 짝 열쇠는 **수량을 뗀** 상품명이다 — 안 떼면 1개·3개가 딴 상품이 된다.
...
try:
    with sync_playwright() as p:
        ...
        if 항목들:
            ...
            본다('🔴 상품명 꽁무니 「3개」를 수량으로 뗀다', 줄['주문수량'] == 3, 줄['주문수량'])
            본다('옵션입수 1 (곱하지 않는다)', 줄['옵션입수'] == 1, 줄['옵션입수'])
            본다('배송메모 = 요청사항', 줄['배송메모'] == '부재시문앞', 줄['배송메모'])
            본다('🔴 짝 열쇠 = 수량 뗀 상품명',
                 줄['원본코드'] == '제로가드닝 팜파스 푸밀라 (그라스) 12cm화분 모종', 줄['원본코드'])
            본다('품목은 안 붙는다 (짝을 지으셔야 한다)', 줄['품목코드'] == '', 줄['품목코드'])
            본다('유통명 자리에 당근 상품명', 줄['유통명'] == 줄['원본코드'])
            본다('갈래 = 새것', 항목들[0]['갈래'] == '새것', 항목들[0]['갈래'])

        print('\n[2-1] 상품명에서 수량 떼기')
        본 = 쪽.evaluate('() => ["팜파스 모종 3개","무늬 몬스테라 12cm화분","라벤더 2 개","5개","모종 10개입"]'
                        '.map(s => ZG.당근.이름과수량(s))')
        본다('「모종 3개」 → 모종 · 3', 본[0] == {'이름': '팜파스 모종', '수량': 3}, 본[0])
        본다('🔴 가운데 숫자는 안 건드린다 (12cm화분)',
             본[1] == {'이름': '무늬 몬스테라 12cm화분', '수량': 1}, 본[1])
        본다('띄어쓴 「2 개」도 뗀다', 본[2] == {'이름': '라벤더', '수량': 2}, 본[2])
        본다('🔴 이름이 통째로 없어지지 않는다 (「5개」)', 본[3] == {'이름': '5개', '수량': 1}, 본[3])
        본다('「10개입」은 수량이 아니다', 본[4] == {'이름': '모종 10개입', '수량': 1}, 본[4])

        print('\n[3] 짝창에 당근 줄이 뜨는가')
        글 = 쪽.evaluate('it => { var d = ZG.짝창.상자(it); return d ? d.textContent : ""; }', 항목들)
        본다('짝 못 지은 당근 줄이 상자에 올라온다', '제로가드닝 팜파스' in 글, 글[:80])
        본다('🔴 짝 상자에는 수량 뗀 이름이 뜬다 (「3개」가 안 붙는다)', '모종 3개' not in 글)
